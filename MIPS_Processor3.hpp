...
//-----------------------------------------------------------------------------
			...
			if(IDstage2->stage_empty==false && RRstage->stage_empty==true && IDstage2->reading_done==true ){
				...
				command = commands[RRstage->PCcurr];
				//brach instruciton is our memomy instuction does not matter much
			...
			if(IDstage1->stage_empty==false && IDstage2->stage_empty==true || IDstage2->stage_empty==false){
				...
				|| command[0]=="and"   || command[0]=="or" || command[0]=="xor"){
					...
					if((command[0]=="addi"||command[0]=="sll"||command[0]=="srl") && reg_to_be_updated[command[2] ]==0 
					|| command[0]!="addi"&& command[0]!="sll" && command[0]!="srl"&&reg_to_be_updated[command[2]]==0 && reg_to_be_updated[command[3]]==0  ){
